...
namespace arduinobot_cpp_examples
{
    ...
    public:
        ...
        {
            RCLCPP_INFO(get_logger(), "Starting fibonacci action cpp server");
    ...
    private:
        ...
        {
            RCLCPP_INFO_STREAM(get_logger(), "Goal request - order: " << goal->order);
            (void)uuid;
            return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
        }

        rclcpp_action::CancelResponse cancelCallback(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::Fibonacci>> goalHandle)
        {
        ...
        void acceptedCallback(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::Fibonacci>> goalHandle)
        {
            std::thread(std::bind(&SimpleActionServer::execute, this, _1), goalHandle).detach();
        }
